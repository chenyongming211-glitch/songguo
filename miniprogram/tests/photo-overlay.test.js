...
assert.ok(homeScript.includes("previewImageUrl"), "photo draft should preserve the deskewed preview image URL");
...
assert.ok(
  script.includes("pendingDraft.previewImageUrl || pendingDraft.localImagePath"),
  "submission page should prefer the deskewed server preview over the raw local photo"
);
...
assert.ok(script.includes("routeEvidencePoints"), "submission page should build route evidence view models");
assert.ok(script.includes("evidencePoints"), "submission page should expose per-item evidence points");
...
assert.ok(script.includes('warning === "photo_not_level"'), "submission page should name non-level photo warnings");
assert.ok(apiScript.includes("ocr_judgement"), "submission API should preserve OCR native judgement");
assert.ok(apiScript.includes("marking_source"), "submission API should preserve OCR marking source");
assert.ok(markup.includes("photo-overlay-panel"), "submission page should render a photo overlay panel");
assert.ok(markup.includes('src="{{photoImagePath}}"'), "submission page should show the OCR-aligned preview photo");
...
assert.ok(markup.includes("route-summary"), "submission page should render auto-route evidence");
assert.ok(markup.includes("item-evidence"), "submission page should render item-level evidence");
...
assert.ok(styles.includes(".route-summary"), "styles should include route evidence state");
assert.ok(styles.includes(".item-evidence"), "styles should include item evidence state");
